...
exports.default = (0, util_1.createRule)({
    ...
    meta: {
        type: 'suggestion',
        docs: {
            description: 'Requires an attributes option when calling .findOne(), .findAll(), or .findByPk() to avoid excessive database load',
            requiresTypeChecking: false,
        },
        messages: {
            requireFindByPkAttributes: 'Always pass an attributes option to .findOne(), .findAll(), or .findByPk() to avoid fetching all columns',
        },
        schema: [],
    },
    create(context) {
        return {
            CallExpression: (node) => {
                if (node.callee.type !== 'MemberExpression' ||
                    ...
                    !['findAll', 'findByPk', 'findOne'].includes(node.callee.property.name)) {
                    return;
                }
                const optionsArgumentIndex = node.callee.property.name === 'findByPk' ? 1 : 0;
